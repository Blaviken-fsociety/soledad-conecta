...
export default function EntrepreneurPanel() {
  ...
  return (
    <main className="portal portal-service-page portal-dashboard-page entrepreneur-page">
        ...
        <div className="container-xxl">
          <div className="navbar-shell home-toolbar-shell">
            ...
            <div className="navbar-actions-slot">
              <nav className="header-actions service-header-actions home-toolbar-actions dashboard-header-actions" aria-label="Navegacion emprendedor">
                <button
                  type="button"
                  className="btn portal-btn portal-btn-dark logout-button"
                  onClick={() => {
                    clearSession();
                    navigate('/');
                  }}
        ...
        <div className="container-xxl">
          <div className="admin-hero dashboard-hero-grid">
            <div className="hero-copy dashboard-hero-copy">
              ...
              <h1>Microtienda, productos y metricas con una vista mas clara de gestion.</h1>
              <p className="hero-text">
                Espacio del emprendedor para administrar la microtienda digital,
                productos y revisar indicadores reales del negocio con una
                visual mas consistente con el portal publico.
              </p>
            </div>
            ...
            <aside className="hero-panel dashboard-hero-panel">
              ...
              <p>
                Administra precios, inventario, imagenes, logo e informacion
                visible del negocio desde un solo flujo de trabajo.
              </p>
              <p>
      ...
      {mustChangePassword ? (
        <section className="users-section service-section dashboard-section" data-reveal="fade-up">
          <div className="container-xxl">
            ...
            <form className="admin-user-form dashboard-form-card" onSubmit={handleChangePassword}>
              <input
                type="password"
      ...
      {mustChangePassword ? (
        ...
        <>
          <section className="microstore-section service-section dashboard-section" data-reveal="fade-up">
            <div className="container-xxl">
              <div className="section-heading">
                <p className="section-kicker">Microtienda empresarial</p>
                <h2>Gestiona la informacion principal de tu negocio</h2>
              </div>

              <div className="admin-user-layout dashboard-split-layout">
                <form className="admin-user-form dashboard-form-card" onSubmit={handleSaveMicrotienda}>
                  <div className="section-heading">
                    <p className="section-kicker">{microtienda ? 'Edicion' : 'Registro'}</p>
                    <h3>{microtienda ? 'Actualizar microtienda' : 'Crear microtienda'}</h3>
                  </div>

                  <input className="form-control" type="text" placeholder="Nombre del negocio" value={microtiendaForm.nombre} onChange={(event) => setMicrotiendaForm((current) => ({ ...current, nombre: event.target.value }))} />
                  <textarea className="form-control" rows="4" placeholder="Descripcion" value={microtiendaForm.descripcion} onChange={(event) => setMicrotiendaForm((current) => ({ ...current, descripcion: event.target.value }))}></textarea>
                  <input className="form-control" type="text" placeholder="Sector economico" value={microtiendaForm.sectorEconomico} onChange={(event) => setMicrotiendaForm((current) => ({ ...current, sectorEconomico: event.target.value }))} />
                  <input className="form-control" type="text" placeholder="WhatsApp" value={microtiendaForm.whatsapp} onChange={(event) => setMicrotiendaForm((current) => ({ ...current, whatsapp: event.target.value }))} />
                  <input className="form-control" type="text" placeholder="Redes sociales" value={microtiendaForm.redesSociales} onChange={(event) => setMicrotiendaForm((current) => ({ ...current, redesSociales: event.target.value }))} />
                  ...
                  <select className="form-select" value={microtiendaForm.idCategoria} onChange={(event) => setMicrotiendaForm((current) => ({ ...current, idCategoria: event.target.value }))}>
                  ...
                  {microtienda ? (
                    <button type="button" className="btn portal-btn portal-btn-secondary" onClick={handleDeleteMicrotienda}>
                ...
                <div className="microstore-business-card dashboard-card">
          ...
          <section className="users-section service-section dashboard-section" data-reveal="fade-left">
            <div className="container-xxl">
              <div className="section-heading">
                ...
                <h2>Administra el catalogo y el inventario publicado</h2>
              </div>

              <div className="admin-user-layout dashboard-split-layout">
                <form className="admin-user-form dashboard-form-card" onSubmit={handleSaveProduct}>
                  <div className="section-heading">
                    <p className="section-kicker">{productForm.id ? 'Edicion' : 'Registro'}</p>
                    <h3>{productForm.id ? 'Editar producto' : 'Agregar producto'}</h3>
                  </div>

                  <input className="form-control" type="text" placeholder="Nombre del producto" value={productForm.nombre} onChange={(event) => setProductForm((current) => ({ ...current, nombre: event.target.value }))} />
                  <textarea className="form-control" rows="4" placeholder="Descripcion" value={productForm.descripcion} onChange={(event) => setProductForm((current) => ({ ...current, descripcion: event.target.value }))}></textarea>
                  <input className="form-control" type="number" placeholder="Precio" value={productForm.precio} onChange={(event) => setProductForm((current) => ({ ...current, precio: event.target.value }))} />
                  <input className="form-control" type="number" placeholder="Inventario" value={productForm.stock} onChange={(event) => setProductForm((current) => ({ ...current, stock: event.target.value }))} />
                  <label className="upload-field">
                  ...
                  <select className="form-select" value={productForm.idCategoria} onChange={(event) => setProductForm((current) => ({ ...current, idCategoria: event.target.value }))}>
                    <option value="">Selecciona una categoria</option>
                    {categories.map((category) => (
                      <option key={category.id} value={category.id}>
                        {category.nombre}
                  ...
                  <button type="submit" className="btn portal-btn portal-btn-primary" disabled={!microtienda}>
                      ...
                      {products.length > 0 ? (
                        products.map((product) => (
                          <tr key={product.id}>
                            ...
                            <td>
                              <button type="button" className="btn portal-btn portal-btn-secondary" onClick={() => setProductForm(product)}>Editar</button>
                              <button type="button" className="btn portal-btn portal-btn-secondary" onClick={() => handleDeleteProduct(product.id)}>Eliminar</button>
                            </td>
                          </tr>
                        ))
                      ) : (
          ...
          <section className="metrics-section service-section dashboard-section" data-reveal="fade-up">
            <div className="container-xxl">
              <div className="section-heading">
                <p className="section-kicker">Metricas del emprendimiento</p>
                <h2>Indicadores clave de tu actividad comercial</h2>
              </div>
